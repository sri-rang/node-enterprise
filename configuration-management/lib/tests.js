...
const config_management = require('./index');

function should_initialize_data_source() {
    config_management.initialize('test_data.json', {});
    const actual = JSON.stringify(config_management.get());
    const expected = '{}';
    assert(actual === expected);
}

function should_add_hello_world() {
